...
void FileSelectWidget::askNewFile()
{
    ...
    if (m_openingMode)
    {
        newFile = QFileDialog::getOpenFileName(parentWidget(), tr("Sélectionnez un plateau à ouvrir"), "data/", tr("Plateau MonopolAdventure") + " (*" + EXTENSION_FICHIER + ")");
    }
    else
    {
        newFile = QFileDialog::getSaveFileName(parentWidget(), tr("Sélectionnez un fichier à enregistrer"), "data/", tr("Plateau MonopolAdventure") + " (*" + EXTENSION_FICHIER + ")");
    }
